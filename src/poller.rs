...
impl DataPoller {
    pub fn new(
        ...
        let initial_secrets = secrets.lock().unwrap().clone();
        Self {
    ...
    async fn fetch_private_data(&self) {
        ...
        // 1. Binance
        if let (Some(key), Some(secret)) = (&secrets.binance_key, &secrets.binance_secret) {
        ...
        // 3. Bitget
        if let (Some(key), Some(secret)) = (&secrets.bitget_key, &secrets.bitget_secret) {
        ...
        // 5. OKX
        if let (Some(key), Some(secret), Some(passphrase)) = (&secrets.okx_key, &secrets.okx_secret, &secrets.okx_passphrase) {
    ...
    async fn fetch_generic_filters(&self, eid: ExchangeId) -> Result<HashMap<String, crate::model::SymbolMarketFilters>, Box<dyn std::error::Error>> {
        let url = match eid {
            ExchangeId::MEXC => "https://fapi.mexc.com/api/v1/contract/detail",
            ExchangeId::Gate => "https://api.gateio.ws/api/v4/futures/usdt/contracts",
            ExchangeId::Bitmart => "https://api-cloud.bitmart.com/futures/OT/v1/instruments",
            ExchangeId::Kraken => "https://futures.kraken.com/derivatives/api/v3/instruments",
            ExchangeId::Ourbit => "https://api.ourbit.com/api/v1/contract/detail",
