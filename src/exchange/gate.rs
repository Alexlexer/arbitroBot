...
const GATE_SUB_BATCH_SIZE: usize = 20;
const GATE_SUB_BATCH_DELAY_MS: u64 = 100;
const GATE_PING_INTERVAL_SECS: u64 = 15;
...
impl Exchange for GateLauncher {
    async fn connect(
        ...
        tokio::spawn(async move {
            let url = Url::parse(GATE_WS_URL).expect("Invalid Gate.io WebSocket URL");
            ...
            for attempt in 1..=5 {
                ...
                match fetch_gate_futures_contracts(&client).await {
                    Ok(list) => {
                        contracts = list;
                    }
            ...
            if contracts.len() > GATE_DEPTH_SYMBOLS_LIMIT {
                contracts.truncate(GATE_DEPTH_SYMBOLS_LIMIT);
            }
            ...
            loop {
                ...
                match connect_async(url.clone()).await {
                    Ok((ws_stream, _)) => {
                        ...
                        // Subscribe in batches — one message per contract
                        for chunk in contracts.chunks(GATE_SUB_BATCH_SIZE) {
                            ...
                            tokio::time::sleep(tokio::time::Duration::from_millis(GATE_SUB_BATCH_DELAY_MS)).await;
                        }
                        ...
                        tokio::spawn(async move {
                            loop {
                                tokio::time::sleep(tokio::time::Duration::from_secs(GATE_PING_INTERVAL_SECS)).await;
                                let now = chrono::Utc::now().timestamp();
                                let ping = json!({
                                    "time": now,
                        ...
                        while let Some(msg) = read.next().await {
                            let text = match msg {
                                ...
                                Ok(Message::Close(_)) => {
                                    info!("Gate: WebSocket closed by server.");
                                    break;
                                }
                                Err(e) => {
                                    error!("Gate: WS read error: {}", e);
                                    break;
                                }
                                _ => continue,
                            };
