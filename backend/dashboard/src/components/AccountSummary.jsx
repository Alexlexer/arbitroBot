            ...
            <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden group">
                ...
                <div className="relative z-10">
                    ...
                    <div className="text-4xl font-extrabold tracking-tight mb-4 tabular-nums">
                        ${state.total_equity_usdt.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </div>
                    <div className="flex gap-4 items-center">
                        <div className="bg-white/10 rounded-lg px-4 py-2 backdrop-blur-md">
                            ...
                            <div className={`font-mono font-bold ${state.total_unrealized_pnl >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>
                                {state.total_unrealized_pnl >= 0 ? '+' : ''}${state.total_unrealized_pnl.toFixed(2)}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    {Object.entries(state.exchange_states).map(([ex, s]) => (
                        <div key={ex} className="p-4 rounded-xl bg-slate-800/30 border border-slate-800/50 hover:border-slate-700 transition-colors group">
                            ...
                            <div className="text-lg font-bold text-white tabular-nums">${s.total_equity.toFixed(0)}</div>
                            <div className="w-full bg-slate-800 h-1.5 rounded-full mt-3 overflow-hidden">
                                <div
                                    className={`h-full transition-all duration-1000 ${s.margin_ratio > 0.7 ? 'bg-rose-500' : s.margin_ratio > 0.4 ? 'bg-amber-500' : 'bg-emerald-500'
                                        }`}
                                    style={{ width: `${(s.margin_ratio * 100).toFixed(0)}%` }}
                                />
                            </div>
                            <div className="flex justify-between mt-1">
                                <span className="text-[9px] text-slate-600 font-bold uppercase">Margin</span>
                                <span className="text-[9px] text-slate-400 font-mono font-bold">{(s.margin_ratio * 100).toFixed(1)}%</span>
                            </div>
                        </div>
                    ))}
                </div>
